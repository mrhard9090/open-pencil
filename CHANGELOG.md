...
- Save each layer of an imported `.fig` with its own sibling order key after layers are added or reordered, so Figma keeps the order you see instead of finding two layers with the same key (#766).
